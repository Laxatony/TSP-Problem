...
def expendAvailabelStateNode(curStateNode):
	...
	for cityIndex in unVisitedCityList:
		...
		if not visitedCityList in g_historicalState:
			nextStateNode = stateNode(visitedCityList)
			g_historicalState.append(visitedCityList)
			heappush(g_myPQ_bestStateNode, (nextStateNode.fCost, nextStateNode.gCost, nextStateNode.hCost, visitedCityList, nextStateNode))
...
def AStar_Algorithm():
	...
	while len(curStateNode.unVisitedCityList) != 0:
...
def main():
	...
	for i, city_coordinates in enumerate(cityCoordsSet):
		...
		g_historicalState.append(visitedCityList)
		heappush(g_myPQ_bestStateNode, (initialStateNode.fCost, initialStateNode.gCost, initialStateNode.hCost, visitedCityList, initialStateNode))
		g_totalCreatedNode += 1
	
		resultStateNode = AStar_Algorithm()
